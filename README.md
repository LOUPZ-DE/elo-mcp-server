...
| `elo_get_document_content` | Extracts the text of a PDF, Word, Excel, e-mail (`.eml`/`.msg`) or plain-text document — any version, by `versionId` |
| `elo_get_metadata` | Index fields, mask, owner, dates and the **full version history** — each entry with comment, timestamp, size and a `versionId` |
