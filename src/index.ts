  ...
  if (cfg.ELO_DOCUMENT_CONTENT_ENABLED) {
    server.registerTool(
      'elo_get_document_content',
      {
        ...
        description:
          'Returns the extracted text of a document — this is how you read what is actually inside a PDF, Word file, e-mail (.eml) or text file in ELO. Use it whenever a question is about document *content* rather than about which documents exist.\n\n' +
          ...
          'For an .eml the result begins with a From/To/Subject/Date block, then the message body; attachments are listed by name only, and are usually filed as separate ELO documents you can read individually.\n\n' +
          'Scanned PDFs have no text layer and return empty text with `textLayer: "none"` — say so rather than guessing at the content. Spreadsheets, presentations, Outlook `.msg`, ELO containers (`.ecf`) and images are not readable; the response explains why and gives you the eloLink to pass to the user.\n\n' +
