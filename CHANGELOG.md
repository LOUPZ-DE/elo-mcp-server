...
- **A server icon**, reaching clients two ways because neither covers
  ...
  The loader also tolerates a differently named drop-in: `icon.png` wins when
  present, otherwise a single PNG in `assets/` is used and logged. `icon.png` is
  not the name anyone reaches for when the file is a company logo, and getting
