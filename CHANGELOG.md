...
- **A server icon**, reaching clients two ways because neither covers
  ...
  `assets/icon.png` is generated by `scripts/generate-icon.mjs` — committed, but
  reproducible, so the design can be reviewed as code rather than trusted as a
  blob. It is deliberately **not** the ELO logo, which is a registered trademark
  of ELO Digital Office GmbH and not ours to redistribute in a public
  repository; it draws a neutral filing-stack mark in the LOUPZ palette.
  Replacing it with an official icon means dropping a PNG at `assets/icon.png`
